...
// 1. HOCKEY SCOREBOARD SKELETON
export function HockeyScoreboardSkeleton() {
  return (
    <div className={cardClass} style={cardStyle}>
      <div className="p-6 border-b flex flex-col items-center gap-6 relative" style={{ borderColor: "var(--widget-border)" }}>
         ...
         <div className="flex items-center justify-between w-full max-w-2xl mt-8">
            ...
            {/* Score Center (Periods) */}
            <div className="flex flex-col items-center justify-center gap-2">
               <Skeleton className="h-10 w-24 rounded-lg" /> {/* Total Score */}
               <div className="flex gap-1 mt-1">
                  <Skeleton className="h-4 w-4" /> {/* P1 */}
                  <Skeleton className="h-4 w-4" /> {/* P2 */}
                  <Skeleton className="h-4 w-4" /> {/* P3 */}
               </div>
...
// 2. HOCKEY H2H SKELETON
export function HockeyH2HSkeleton() {
  return (
...
// 3. HOCKEY STANDINGS SKELETON
export function HockeyStandingsSkeleton() {
  return (
    <div className="p-4 space-y-8">
      {[1, 2].map((g) => (
        <div key={g} className="space-y-4">
           <Skeleton className="h-6 w-40" /> {/* Division/Group Name */}
...
// 4. HOCKEY ODDS SKELETON
export function HockeyOddsSkeleton() {
  return (
        ...
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className={`p-3 ${cardClass}`} style={cardStyle}>
             ...
             <div className="flex gap-2">
                {[1, 2, 3].map((j) => ( /* 3-Way (Home/Draw/Away) */
                   <div 
                     key={j} 

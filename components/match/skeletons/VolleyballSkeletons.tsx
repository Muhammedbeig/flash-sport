...
// 1. VOLLEYBALL SCOREBOARD SKELETON
export function VolleyballScoreboardSkeleton() {
  return (
    <div className={cardClass} style={cardStyle}>
      <div className="p-6 border-b flex flex-col items-center gap-6 relative" style={{ borderColor: "var(--widget-border)" }}>
         ...
         <div className="flex items-center justify-between w-full max-w-2xl mt-8">
            ...
            <div className="flex flex-col items-center justify-center gap-2">
               <Skeleton className="h-10 w-24 rounded-lg" /> {/* Total Sets */}
               <div className="flex gap-1 mt-1">
                  <Skeleton className="h-4 w-4" /> {/* S1 */}
                  <Skeleton className="h-4 w-4" /> {/* S2 */}
                  <Skeleton className="h-4 w-4" /> {/* S3 */}
                  <Skeleton className="h-4 w-4" /> {/* S4 */}
                  <Skeleton className="h-4 w-4" /> {/* S5 */}
               </div>
               <Skeleton className="h-4 w-16 rounded-full mt-1" /> {/* Status */}
            </div>
      ...
      <div className="flex items-center gap-2 px-4 border-b h-12" style={{ borderColor: "var(--widget-border)" }}>
         {[1, 2, 3, 4, 5].map(i => <Skeleton key={i} className="h-8 w-20 rounded-lg shrink-0" />)}
      </div>
...
// 2. VOLLEYBALL H2H SKELETON
export function VolleyballH2HSkeleton() {
  return (
...
// 3. VOLLEYBALL STANDINGS SKELETON
export function VolleyballStandingsSkeleton() {
  return (
    <div className="p-4 space-y-8">
      {[1, 2].map((g) => (
        <div key={g} className="space-y-4">
           <Skeleton className="h-6 w-40" /> {/* Group Name */}
...
// 4. VOLLEYBALL ODDS SKELETON
export function VolleyballOddsSkeleton() {
  return (
        ...
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className={`p-3 ${cardClass}`} style={cardStyle}>
             ...
             <div className="flex gap-2">
                {[1, 2].map((j) => ( /* 2-Way (Home/Away) is common in Volleyball */
                   <div 
                     key={j} 

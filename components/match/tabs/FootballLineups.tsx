...
export default function FootballLineups({ lineups, error }: { lineups: TeamLineup[], error?: string | null }) {
  ...
  const renderList = (players: Player[], title: string) => (
    <div className="mb-6">
      <h4 className="text-xs font-bold uppercase text-secondary mb-3 tracking-wider border-b theme-border pb-2">{title}</h4>
      <div className="space-y-1">
        {(players || []).map((item, idx) => { 
            ...
            return (
                <div key={pId || idx} className={`flex items-center justify-between text-sm p-2 rounded transition-colors ${rowHover}`}>
                    <div className="flex items-center gap-3">
                        <span className="w-6 text-center font-mono font-bold text-secondary opacity-60">{item.player.number}</span>
                        {pId ? (
                             <Link href={`/player?id=${pId}`} target="_blank" className="font-medium text-primary hover:text-blue-500 hover:underline">
                                {pName}
                             </Link>
                        ) : (
        ...
        {(!players || players.length === 0) && <div className="text-xs text-secondary italic px-2">No data</div>}
      </div>
    </div>
  );
        ...
        <div className="flex items-center gap-3 mb-6 pb-4 border-b theme-border">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-xs text-secondary">Coach: <span className="text-primary font-medium">{home.coach?.name}</span></span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 mb-6 pb-4 border-b theme-border">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-xs text-secondary">Coach: <span className="text-primary font-medium">{away.coach?.name}</span></span>
            </div>
          </div>
        </div>

...
export default function RugbyMatchWidget({ matchId, initialTab }: { matchId: string, initialTab?: string }) {
  ...
  useEffect(() => {
    if (initialTab === "summary" || initialTab === "h2h") {
  ...
  useEffect(() => {
    async function fetchMatch() {
      ...
      try {
        ...
        if (data) {
          // Handle both structures (game/league nested vs flat)
          ...
          setMatch({
            ...
            league: {
              ...
              country: leagueInfo.country?.name || leagueInfo.country,
  ...
  const isLive = ["1H", "2H", "HT"].includes(status.short);
  ...
  return (
    <div className="theme-bg rounded-xl border theme-border overflow-hidden">
      ...
      <div className="flex items-center gap-1 px-4 border-b theme-border">
        <Link href={`/match?id=${matchId}&sport=rugby/summary`} replace={true} prefetch={false} className={`${tabBase} ${activeTab === "summary" ? activeClass : inactiveClass}`}>Summary</Link>
        <Link href={`/match?id=${matchId}&sport=rugby/h2h`} replace={true} prefetch={false} className={`${tabBase} ${activeTab === "h2h" ? activeClass : inactiveClass}`}>H2H</Link>
      </div>
      <div className="min-h-[300px]">
        {activeTab === "summary" && <RugbySummary match={match} />}
        {activeTab === "h2h" && <MatchH2H teamOneId={teams.home.id} teamTwoId={teams.away.id} sport="rugby" />}
      </div>
    </div>
  );
}
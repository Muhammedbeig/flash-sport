...
type VolleyballMatch = {
  ...
  scores: any; // Allow flexible score object for sets
};
  ...
  return (
    <div className="theme-bg rounded-xl border theme-border overflow-hidden">
      ...
      <div className="flex items-center gap-1 px-4 border-b theme-border">
        <Link href={`/match?id=${matchId}&sport=volleyball/summary`} replace={true} prefetch={false} className={`${tabBase} ${activeTab === "summary" ? activeClass : inactiveClass}`}>Summary</Link>
        <Link href={`/match?id=${matchId}&sport=volleyball/h2h`} replace={true} prefetch={false} className={`${tabBase} ${activeTab === "h2h" ? activeClass : inactiveClass}`}>H2H</Link>
      </div>
      <div className="min-h-[300px]">
        {activeTab === "summary" && <VolleyballSummary match={match} />}
        {activeTab === "h2h" && <MatchH2H teamOneId={teams.home.id} teamTwoId={teams.away.id} sport="volleyball" />}
      </div>
    </div>
  );
}
...
export default function GameWidget({ sport = "football" }: GameWidgetProps) {
  const { theme } = useTheme();
  
  // Supported sports for the Custom Feed (Team sports with Home vs Away structure)
  const CUSTOM_FEED_SPORTS = ['football', 'basketball', 'baseball', 'hockey', 'rugby', 'nfl', 'nba', 'volleyball', 'handball'];

  // If the sport is supported by our custom feed, use it.
  if (CUSTOM_FEED_SPORTS.includes(sport)) {
    return (
      <div className="w-full min-h-[500px]">
        <CustomGameFeed sport={sport} />
      </div>
    );
  }
  ...
  return (
    <div className="w-full min-h-[500px] theme-bg rounded-xl border theme-border p-4">
       <div dangerouslySetInnerHTML={{
          __html: `<api-sports-widget data-type="games" data-sport="${sport}" data-theme="${widgetTheme}" data-show-toolbar="true" data-refresh="60"></api-sports-widget>`
       }} />
    </div>
  );
}
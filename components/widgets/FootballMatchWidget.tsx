    ...
    <div className="theme-bg rounded-xl border theme-border overflow-hidden">
      ...
      <div className="p-6 border-b theme-border flex flex-col items-center gap-6 relative overflow-hidden">
        ...
        <div className="flex items-center justify-between w-full max-w-2xl mt-8">
          ...
          <div className="flex flex-col items-center justify-center gap-2">
            <div className="text-4xl font-black text-primary tracking-tight font-mono">{goals.home ?? "-"} : {goals.away ?? "-"}</div>
      ...
      <div className="flex items-center gap-1 px-4 border-b theme-border overflow-x-auto no-scrollbar">
        {validTabs.map(t => {
            ...
            return (
                <Link key={t} href={`/match?id=${matchId}&sport=football/${t}`} replace={true} prefetch={false} className={`${tabBase} ${activeTab === t ? activeClass : inactiveClass}`}>
                  {label}
                </Link>
            )
        })}
      ...
      <div className="min-h-[300px]">
        ...
        {/* Pass IDs only if teams exist */}
        {activeTab === "h2h" && teams.home?.id && teams.away?.id && (
            <FootballH2H teamOneId={teams.home.id} teamTwoId={teams.away.id} />
        )}

        {activeTab === "standings" && league.id && (
            <FootballStandings leagueId={league.id} season={league.season} teamId={teams.home.id} />
        )}
        
        {activeTab === "odds" && fixture.id && (
            <FootballOdds matchId={String(fixture.id)} />
        )}
      </div>
    </div>
  );

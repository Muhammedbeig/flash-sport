...
// --- CONFIGURATION (API-AMERICAN-FOOTBALL) ---
    ...
    <div className="border-b theme-border last:border-0">
      ...
      <div className={`divide-y theme-border ${isOpen ? "block" : "hidden"}`}>
        {games.map((game) => {
          ...
          const displayStatus = isLive ? (game.status.elapsed ? `Q${game.status.short.slice(-1)} ${game.status.elapsed}'` : game.status.short) : (SCHEDULED_CODES.includes(game.status.short) ? time : game.status.short);

          return (
            <Link
              key={game.id}
              href={`/match?id=${game.id}&sport=nfl`}
              target="_blank"
              prefetch={false}
...
export default function NFLFeedUI({ games, loading, leagueId, initialTab }: NFLFeedUIProps) {
  ...
  // Generate URL: ?sport=nfl/live
  const getTabUrl = (tabId: string) => {
    const newSportParam = `nfl/${tabId}`;

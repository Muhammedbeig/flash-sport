...
import { NormalizedGame, NormalizedLeague } from "@/components/feed/utils";
...
function safeMatchHref(gameId: unknown) {
  ...
  if (!id) return null;
  return `/match/nfl/${encodeURIComponent(id)}/summary`;
}
      ...
      <div className={`divide-y theme-border ${isOpen ? "block" : "hidden"}`}>
        {games.map((game) => {
              ...
              <div className="flex-1 px-4 space-y-2">
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-medium text-secondary">{homeName}</span>
                  </div>
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-medium text-secondary">{awayName}</span>
                  </div>
          ...
          if (!href) {
            return (
              <div
                key={String(game.id ?? `${game.teams.home.id}-${game.teams.away.id}-${game.date}`)}
...
export default function NFLFeedUI({ games, loading, leagueId, initialTab }: NFLFeedUIProps) {
  ...
  const grouped = filteredGames.reduce<Record<string, { meta: NormalizedLeague; games: NormalizedGame[] }>>(
    (groups, game) => {
      const key = `${game.league.country || "USA"}-${game.league.name}`;
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="theme-bg rounded-xl border theme-border overflow-hidden shadow-sm">
        ...
        {filteredGames.length === 0 && <div className="p-8 text-center text-secondary">No matches found.</div>}
      </div>
    </div>
  );
}
...
// Matches your League JSON structure
type SeoLeagueDoc = {
  labels?: {
...
export default function LeagueSeoClient() {
  const router = useRouter();
  const BYPASS = process.env.NEXT_PUBLIC_ADMIN_BYPASS_AUTH === "true";
  const { user, loading } = useAdminAuth();

  const [data, setData] = useState<SeoLeagueDoc | null>(null);
  const [base, setBase] = useState<SeoLeagueDoc | null>(null);
  ...
  async function load() {
    ...
    try {
      ...
      const res = await fetch("/api/seo/league", { method: "GET" });
      if (res.status === 401) {
        router.replace(`${withAdminBase("/login")}?next=${encodeURIComponent(withAdminBase("/seo/league"))}`);
        return;
      }
  ...
  async function save() {
    ...
    try {
      const res = await fetch("/api/seo/league", {
        method: "POST",
  ...
  if (loading || (saveState.kind === "loading" && !data)) {
    return (
      <div className="theme-bg theme-border border rounded-xl p-6">
        <div className="text-sm text-secondary">Loading league settings...</div>
      </div>
    );
  }
  ...
  const l = data.league || {};
  const labels = data.labels?.sportsTabLabels || {};
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="theme-bg theme-border border rounded-xl p-5 flex items-center justify-between sticky top-4 z-30 shadow-sm">
        <div>
          <div className="text-lg font-black text-primary">League SEO</div>
          <div className="text-xs text-secondary">Manage patterns for league/tournament pages.</div>
        </div>
      ...
      {/* Config */}
      <Section title="Configuration" description="Technical settings for league pages.">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              value={l.revalidateSeconds ?? 86400}
              onChange={(e) => set("league.revalidateSeconds", Number(e.target.value))}
            />
          </div>
          <div>
            ...
            <input
              type="number"
              className={inputClass}
              value={l.apiTimeoutMs ?? 2000}
              onChange={(e) => set("league.apiTimeoutMs", Number(e.target.value))}
            />
          </div>
          <div>
            ...
            <input
              className={inputClass}
              value={l.primaryKeyword ?? ""}
              onChange={(e) => set("league.primaryKeyword", e.target.value)}
            />
          </div>
           <div className="flex items-center gap-2 mt-6">
              <input 
                type="checkbox" 
                checked={l.schema?.enabled ?? true}
                onChange={(e) => set("league.schema.enabled", e.target.checked)}
      ...
      {/* Patterns */}
      <Section title="SEO Patterns" description="Variables: {name}, {country}, {season}, {brand}">
        <div className="space-y-4">
          <div>
            ...
            <input
              className={inputClass}
              value={l.h1Pattern ?? ""}
              onChange={(e) => set("league.h1Pattern", e.target.value)}
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              value={l.descriptionPattern ?? ""}
              onChange={(e) => set("league.descriptionPattern", e.target.value)}
            />
          </div>
          ...
          <div>
             ...
             <div className="space-y-2">
                {(l.titlePatterns || []).map((pat, idx) => (
                  <div key={idx} className="flex gap-2">
                    <input 
                      className={inputClass}
                      value={pat}
                      onChange={(e) => {
                        const copy = [...(l.titlePatterns || [])];
                        copy[idx] = e.target.value;
                        set("league.titlePatterns", copy);
                      }}
                    />
                    <button 
                      onClick={() => {
                ...
                <button 
                  onClick={() => set("league.titlePatterns", [...(l.titlePatterns || []), "New Pattern"])}
      ...
      {/* Labels */}
      <Section title="Tab Translations / Labels" description="Customize text for league tabs.">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {Object.entries(labels).map(([key, val]) => (
            <div key={key}>
              ...
              <input
                className={inputClass}
                value={val as string}
                onChange={(e) => set(`labels.sportsTabLabels.${key}`, e.target.value)}
              />
            </div>
          ))}
        </div>
      ...
      <Section title="Open Graph (Social)" description="Social media share settings.">
         <div>
          ...
          <div className="flex gap-2">
            <input
              className={inputClass}
              value={l.og?.fallbackImage ?? ""}
              onChange={(e) => set("league.og.fallbackImage", e.target.value)}
              placeholder="https://..."
            />
            <label className={`
